...
class Game_form(QMainWindow):
    def __init__(self, parent):
        super().__init__()
        uic.loadUi(paths.ui("game_form.ui"), self)
        self.parent = parent
        self.session = Save_session()
        self.reset_icon = QIcon(paths.asset("reset_icon.png"))
        self.button_reset.setIcon(self.reset_icon)
        self.setWindowTitle("Тестовая форма")
        with open(paths.data("english_words.txt"), "r", encoding="utf-8") as f:
            self.english_base = f.readlines()
        with open(paths.data("russian_words.txt"), "r", encoding="utf-8") as f:
    ...
    def new_text(self):
        ...
        if self.language == 'Eng':
            for i in range(self.words_count):
                self.text.append(random.choice(self.english_base).strip())
        else:
            for i in range(self.words_count):
                self.text.append(random.choice(self.russian_base).strip())
        return self.text
    ...
    def end(self):
        ...
        if self.language == "Ru":
            if self.result > int(player_res[1]):
                self.cursor.execute(
                    "UPDATE results SET ru_result=? WHERE user_name=?", (self.result, user_name))
        else:
            if self.result > int(player_res[2]):
                self.cursor.execute(
                    "UPDATE results SET eng_result=? WHERE user_name=?", (self.result, user_name))
